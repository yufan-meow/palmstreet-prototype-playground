<template>
  <div class="root">
    ...
    <template v-if="!showBulkEdit">
      ...
      <div class="toolbar">
        ...
        <button class="btn secondary sm">
          Add Filters <span class="material-symbols-rounded icon-sm">add</span>
        </button>
        <button class="sort-pill">
          Sort: Newest first <span class="material-symbols-rounded icon-sm">expand_more</span>
        </button>
      </div>

      <div v-if="selectedCount > 0" class="bulk-bar">
        <label class="bulk-check-label">
          <input type="checkbox" class="cb" :checked="allSelected" :indeterminate.prop="!allSelected && selectedCount > 0" @change="toggleAll" />
          <span class="bulk-count-label">{{ selectedCount }} selected</span>
        </label>
        <button class="link-btn" @click="deselectAll">Deselect all {{ selectedCount }}</button>
      ...
      <div class="listings-list">
        <div v-for="listing in listings" :key="listing.id" class="listing-row" :class="{ 'row-selected': listing.selected }">
    ...
    <template v-else>
      ...
      <!-- Sticky sub-header — top: 48px (below proto-bar) -->
      <div class="be-header">
        ...
        <div class="be-header-right">
          ...
          <button class="btn primary" :disabled="!hasChanges" @click="saveChanges">Save</button>
        </div>
      </div>

      <!-- .be-scroll owns both scroll axes; thead sticks at top:0 inside it -->
      <div class="be-scroll">
        <table class="be-table">
          ...
          <tbody>
            <tr v-for="(listing, idx) in selectedListings" :key="listing.id" class="be-row" :class="{ 'be-row-editing': editingRow === idx }">
              ...
              <td class="td-cell" :class="{ 'cell-editing': editingRow === idx }" :style="{ width: colWidths.title + 'px' }">
                <input v-if="editingRow === idx" ref="titleInputEl" v-model="listing.title" class="be-title-input"
                  @blur="editingRow = null" @keyup.enter="editingRow = null" @keyup.escape="editingRow = null" @input="hasChanges = true" />
                <div v-else class="be-title-text" @click="startEditTitle(idx)">
                  <span class="title-inner">{{ listing.title }}</span>
                  <span v-if="listing.title.length > 20" class="material-symbols-rounded title-overflow-arrow">chevron_right</span>
                </div>
              </td>
              ...
              <td class="td-cell" :style="{ width: colWidths.price + 'px' }">
                <input v-model="listing.price" type="number" min="0" step="0.01" class="be-price-input" @input="hasChanges = true" />
              </td>
              ...
              <td class="td-cell" :style="{ width: colWidths.sku + 'px' }">
                <input v-model="listing.sku" class="be-sku-input" placeholder="Input SKU here" @input="hasChanges = true" />
              </td>
...
const colWidths = ref({ check: 40, image: 72, title: 220, stock: 110, price: 100, type: 148, vis: 148, ship: 230, sku: 180 })
...
function startResize(e, colKey) {
  ...
  function onMove(e) {
    ...
    colWidths.value[activeResize.value.colKey] = Math.max(48, activeResize.value.startWidth + (e.clientX - activeResize.value.startX))
  }
  function onUp() { activeResize.value = null; window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp) }
...
const allSelected = computed(() => listings.value.length > 0 && listings.value.every(l => l.selected))

function toggleAll(e) { listings.value.forEach(l => (l.selected = e.target.checked)) }
function deselectAll() { listings.value.forEach(l => (l.selected = false)) }
function openBulkEdit() { hasChanges.value = false; showBulkEdit.value = true }
function closeBulkEdit() { showBulkEdit.value = false; editingRow.value = null }
...
function saveChanges() {
  ...
  setTimeout(() => { showToast.value = false; showBulkEdit.value = false }, 1800)
}
</script>

<style scoped>
.root { font-family: 'Gabarito', sans-serif; background: var(--section-basement); min-height: 100vh; color: var(--content-primary); position: relative; }

/* Proto bar — explicit 48px so be-header and thead can offset against it */
.proto-bar { display: flex; align-items: center; justify-content: space-between; padding: 0 24px; height: 48px; border-bottom: 1px solid var(--border); background: var(--section-floor-1); position: sticky; top: 0; z-index: 30; box-sizing: border-box; }
.back-link { display: flex; align-items: center; gap: 6px; font-size: 14px; color: var(--content-interactive); text-decoration: none; }
.proto-label { font-size: 14px; font-weight: 500; color: var(--content-secondary); }
.proto-theme-toggle { width: 32px; height: 32px; border-radius: 8px; border: 1px solid var(--border); background: var(--section-basement); cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--content-primary); }

/* Buttons */
.btn { display: inline-flex; align-items: center; gap: 4px; height: 40px; padding: 0 16px; border-radius: 8px; font-family: 'Gabarito', sans-serif; font-size: 16px; font-weight: 500; cursor: pointer; border: none; transition: filter 0.1s; white-space: nowrap; }
...
.link-btn { background: none; border: none; padding: 0; font-family: 'Gabarito', sans-serif; font-size: 14px; font-weight: 500; color: var(--content-interactive); cursor: pointer; white-space: nowrap; }
.icon-btn { display: flex; align-items: center; justify-content: center; width: 40px; height: 40px; border: none; background: none; cursor: pointer; border-radius: 8px; color: var(--content-primary); flex-shrink: 0; }
...
/* Listings page */
.page-header { display: flex; align-items: center; justify-content: space-between; padding: 24px 32px; border-bottom: 1px solid var(--border); }
...
.tab { padding: 14px 16px; background: none; border: none; border-bottom: 2px solid transparent; margin-bottom: -1px; font-family: 'Gabarito', sans-serif; font-size: 16px; font-weight: 500; color: var(--content-secondary); cursor: pointer; }
...
.search-box { display: flex; align-items: center; gap: 8px; height: 40px; padding: 0 12px; background: var(--section-floor-1); border: 1px solid var(--border); border-radius: 8px; width: 220px; color: var(--content-tertiary); }
...
.sort-pill { display: flex; align-items: center; gap: 4px; margin-left: auto; height: 40px; padding: 0 12px; background: var(--section-floor-1); border: 1px solid var(--border); border-radius: 8px; font-family: 'Gabarito', sans-serif; font-size: 14px; color: var(--content-secondary); cursor: pointer; }
...
.listing-row { display: flex; align-items: center; gap: 16px; padding: 12px 0; border-bottom: 1px solid var(--border); transition: background 0.1s; }
...
/* Bulk edit header — top: 48px (below proto-bar) */
.be-header { display: flex; align-items: center; justify-content: space-between; padding: 0 24px; height: 64px; border-bottom: 1px solid var(--border); background: var(--section-basement); position: sticky; top: 48px; z-index: 20; box-sizing: border-box; }
...
/* Spreadsheet — .be-scroll is the scroll container for both axes.
   height = 100vh - proto-bar (48px) - be-header (64px)           */
.be-scroll { overflow: auto; height: calc(100vh - 112px); }
.be-table { border-collapse: collapse; font-family: 'Gabarito', sans-serif; font-size: 14px; table-layout: fixed; }

/* thead sticks at top: 0 inside .be-scroll */
.be-table thead th { background: var(--section-floor-1); color: var(--content-secondary); font-size: 12px; font-weight: 600; text-align: left; padding: 10px 12px; border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); white-space: nowrap; overflow: hidden; position: sticky; top: 0; z-index: 10; }
.be-table thead th:last-child { border-right: none; }

/* Resize handle */
.resize-handle { position: absolute; right: 0; top: 0; bottom: 0; width: 5px; cursor: col-resize; background: transparent; transition: background 0.1s; }
.resize-handle:hover, .resize-handle:active { background: var(--border-strong); }

/* Body cells */
.td-cell { padding: 8px 12px; border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); vertical-align: middle; background: var(--section-basement); overflow: hidden; }
...
/* Title cell */
.be-title-text { display: flex; align-items: center; gap: 4px; cursor: text; padding: 4px; border-radius: 4px; border: 1px solid transparent; overflow: hidden; }
.be-title-text:hover { border-color: var(--border); background: var(--section-floor-1); }
.title-inner { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--content-primary); }
.title-overflow-arrow { font-size: 16px !important; color: var(--content-tertiary); flex-shrink: 0; line-height: 1; }
.be-title-input { width: 100%; padding: 4px 8px; background: var(--section-floor-1); border: 1.5px solid var(--border-strong); border-radius: 4px; font-family: 'Gabarito', sans-serif; font-size: 14px; color: var(--content-primary); outline: none; box-sizing: border-box; }

.cell-text { color: var(--content-primary); font-size: 14px; }

.be-price-input { width: 100%; background: none; border: none; border-bottom: 1.5px solid transparent; font-family: 'Gabarito', sans-serif; font-size: 14px; color: var(--content-primary); outline: none; padding: 2px 0; box-sizing: border-box; }
...
.be-select { appearance: none; -webkit-appearance: none; background: none; border: none; font-family: 'Gabarito', sans-serif; font-size: 14px; color: var(--content-primary); cursor: pointer; outline: none; flex: 1; min-width: 0; padding-right: 24px; }
.be-chevron { font-size: 18px !important; color: var(--content-secondary); pointer-events: none; position: absolute; right: 0; }

.be-sku-input { width: 100%; background: none; border: none; border-bottom: 1.5px solid transparent; font-family: 'Gabarito', sans-serif; font-size: 14px; color: var(--content-primary); outline: none; padding: 2px 0; box-sizing: border-box; }
...
.toast { position: fixed; top: 24px; left: 50%; transform: translateX(-50%); display: flex; align-items: center; gap: 8px; height: 48px; padding: 0 16px; background: var(--section-floor-1-inverse); border-radius: 12px; z-index: 100; animation: toast-in 0.2s ease; }
